package org.mjjaen.rest.handlerexceptions;
...
public class HandlerExceptionsApplicationTests {	
	@Autowired
	...
	public void test04FindOneNotExists() {
		ResponseEntity<Person> responseEntity = restTemplate.getForEntity("/webservices/person/100", Person.class);
		Person person = responseEntity.getBody();
		assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
	...
	public void test09DeleteNotExists() {
		ResponseEntity<Person> responseEntity = restTemplate.exchange("/webservices/person/100", HttpMethod.DELETE, null, new ParameterizedTypeReference<Person>() {});
		assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
	}
}
